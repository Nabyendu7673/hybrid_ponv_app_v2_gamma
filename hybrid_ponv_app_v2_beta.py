...
def calculate_hybrid_score():
    ...
    score += binary(gender)
    score += binary(smoker)
    score += binary(history_ponv)
    score += binary(preop_anxiety)
    score += binary(history_migraine)
    score += binary(obesity)
    score += binary(abdominal_surgery)
    score += binary(ent_surgery)
    score += binary(gynae_surgery)
    score += binary(surgery_duration)
    score += binary(major_blood_loss)
    score += binary(volatile_agents)
    score += binary(nitrous_oxide)
    score += propofol_score(propofol_mode)
    score -= 1 if midazolam_dose > 0 else 0
    score -= 2 if ondansetron_dose >= 4 else 0
    score -= 2 if dexamethasone_dose >= 4 else 0
    score -= 1 if glycopyrrolate_dose > 0 else 0
    score += 1 if nalbuphine_dose > 0 else 0
    score += 1 if fentanyl_dose > 100 else 0
    score += 1 if butorphanol_dose > 0 else 0
    score += 1 if pentazocine_dose > 0 else 0
    score += binary(nausea)
    score += binary(vomiting)
    score += binary(abdo_discomfort)
    return score

def risk_category(score):
    if score <= -5:
        return "Very Low Risk", "#00cc44"  # Green
    elif -4 <= score <= 3:
        return "Low Risk", "#66ccff"  # Light Blue
    elif 4 <= score <= 9:
        return "Moderate Risk", "#ffcc00"  # Yellow
    elif 10 <= score <= 15:
        return "High Risk", "#ff944d"  # Orange
    else:
        return "Very High Risk", "#ff4d4d"  # Red

# ------------------------- DISPLAY HYBRID SCORE -------------------------
hybrid_score = calculate_hybrid_score()
category, color = risk_category(hybrid_score)

st.subheader("🧮 Hybrid PONV Score Summary")
st.markdown(f"""
<div style='padding: 1em; border-radius: 10px; background-color: {color}; color: black;'>
